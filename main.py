...
class EulerSolution(Solution):
    def Calculate_points(self):
        ...
        for i in range(1, self.grid.n):
            y = y + dy(self.grid.x_points[i], y) * self.grid.h
            self.y_points.append(y)
        return self.y_points
...
class Grid:
    ...
    def Update(self):
        ...
        self.xf = float(mpl_widget.lineEdit_3.text())
        self.n = int(mpl_widget.lineEdit_4.text())
        self.h = float(((self.xf - self.xi) / self.n))
...
class LTE_Grid:
    ...
    def Calculate_LTE(self, exact, euler):
        self.x_points = exact.x_points
        self.y_euler = []
        self.y_euler.append(exact.y_points[0] - euler.y_points[0])
        n = len(self.x_points)
        for i in range(0, n - 1):
            y = abs(exact.y_points[i] - euler.y_points[i])
            self.y_euler.append(y)
        # print("LTE:", self.x_points)
        # print(self.y_euler)
        # print(exact.y_points)
        # print(euler.y_points)
        # print(len(exact.x_points), len(self.y_euler))
        return self.y_euler


class GTE_Grid:
    def __init__(self, mpl_widget, grid):
        self.x_points = []
        self.y_euler = []
...
class MatplotlibWidget(QMainWindow):
    def __init__(self):
        ...
        self.GTE_grid = GTE_Grid(self, self.grid)
...
class Controller:
    ...
    def update_graph(self):
        ...
        y_euler_lte = mpl_widget.LTE_grid.Calculate_LTE(mpl_widget.exactSolution, mpl_widget.eulerSolution)
        ...
        mpl_widget.euler_lte_line, = mpl_widget.SolutionsWidget.canvas.axe_2.plot(x_points, y_euler_lte, ".",
                                                                                  ...
                                                                                  label="Euler",
                                                                                  markersize=4)
        mpl_widget.SolutionsWidget.canvas.axe_1.legend(loc='upper left')
    ...
    def update_GTE(self):
        mpl_widget = self.mpl_widget
        mpl_widget.grid.Update()
        print(mpl_widget.grid.xi)
        x_points, y_points = mpl_widget.GTE_grid.Calculate_GTE()
        mpl_widget.SolutionsWidget.canvas.axe_3.clear()
